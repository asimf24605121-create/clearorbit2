...
- **User & Subscription Management (Single-Page Workflow)**: Users module provides a unified single-page experience with 3 distinct action workflows — no separate Subscriptions page.
  ...
  - **View Profile Modal**: Read-only user details with subscription summary. "Manage Access" link opens the Access modal via `vuGoToSubscriptions()`.
  - **Edit User Modal**: Profile-only fields (name, email, password, phone, profile image). No platform assignment.
