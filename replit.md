...
- **User & Subscription Management (Single-Page Workflow)**: Users module provides a unified single-page experience with 3 distinct action workflows — no separate Subscriptions page.
  ...
  - **IP Geolocation**: `POST /api/geo_lookup` endpoint queries ip-api.com with proxy/hosting detection, 4s timeout, `IpGeoCache` DB (72h TTL), graceful fallback. Client-side `_geoCache` Map + `_geoLookupVersion` race guard.
