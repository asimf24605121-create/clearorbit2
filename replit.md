...
### Key Features
- **User & Subscription Management (Separated Modules)**: Users and Subscriptions are separated into distinct pages for focused management.
  - **Users Page** (`sec-users`): Identity/profile management — create, edit profile, disable, delete. View modal shows read-only subscription summary (active/expired counts + platform chips with badges) with "Manage Access" link to Subscriptions page. Create user form retains optional platform access for onboarding.
  - **Subscriptions Page** (`sec-subscriptions`): Dedicated platform access management — search (by username), filter (status: active/expiring_soon/expired/revoked, platform), sort (expiry asc/desc, newest, by user/platform), pagination. Table with checkbox selection, inline extend (+7d/+30d) and revoke actions, bulk extend/revoke for selected rows. "Assign New Subscription" form (user dropdown, platform dropdown, duration preset). Backend: enhanced `get_subscriptions` with search/sort/status/user_id filters + `days_left`/`computedStatus` in response. `bulk_extend_subscriptions` and `bulk_revoke_subscriptions` endpoints with activity logging. IDOR fix on `get_user_subscriptions` (user_id override restricted to admin roles).
