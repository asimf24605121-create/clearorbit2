...
- **User & Subscription Management (Single-Page Workflow)**: Users module provides a unified single-page experience with 3 distinct action workflows — no separate Subscriptions page.
  - **Users Page** (`sec-users`): 5-column table (User, Email, Expiry, Status, Actions) with View/Edit/Access/Toggle/Delete action buttons. Access icon (purple ticket) opens per-user Manage Access modal.
  - **Manage Access Modal** (`accessModal`): Per-user subscription management via `openAccessModal(userId)`. Active subscriptions with inline extend (+7d/+30d) and revoke per-card. Expired/revoked section. "Add New Subscription" form (platform dropdown, duration preset). Auto-refreshes parent users table. Escape key and backdrop click to close.
  - **View Profile Modal**: Read-only user details (account info, access & security, subscriptions, active slots). Shows IP geolocation (country/city/ISP with flag) via async `vuLookupGeo` with `_geoCache` client-side + server-side `IpGeoCache` DB (72h TTL). Footer has "Manage Access" CTA + "Edit Profile" button. "Manage Access" link also opens the Access modal via `vuGoToSubscriptions()`.
  - **Edit User Modal**: Profile-only fields (name, email, phone, expiry, country, city, gender, password). Save button has loading state. No platform assignment.
  - **IP Geolocation**: `POST /api/geo_lookup` endpoint in admin.js queries ip-api.com with 4s timeout, caches in `IpGeoCache` table (72h TTL), graceful fallback to stale cache on timeout. Client-side `_geoCache` Map prevents repeated lookups. Race-condition guard (`_geoLookupVersion`) prevents stale async overwrites.
